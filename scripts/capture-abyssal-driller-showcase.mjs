...
const SHOWCASE_URL = process.env.SHOWCASE_URL ?? 'http://127.0.0.1:5173/social/abyssal-driller-showcase.html'
const WIDTH = 1920
const HEIGHT = 1080
const FPS = 30
...
const FRAME_DIR = join(tmpdir(), `abyssal-driller-showcase-frames-${Date.now()}`)
const OUT_MP4 = join(OUT_DIR, 'abyssal-driller-showcase.mp4')
const OUT_POSTER = join(OUT_DIR, 'abyssal-driller-showcase-poster.png')
...
async function main() {
  ...
  const profile = join(tmpdir(), `chrome-abyssal-driller-${Date.now()}`)
