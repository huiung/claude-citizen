...
export interface PadSpec { x: number; z: number }
export interface CityLayout { buildings: BuildingSpec[]; pads: PadSpec[] }

/** Dense block-grid: an asphalt pad under EVERY block inside the footprint (parks stay
 *  paved plazas — the 24u gaps between pads read as the street grid), 3-5 buildings per
 *  built block with tight 12-32u footprints, power-law heights peaking downtown. */
export function computeCityLayout(siteSeed: number, tier: 0 | 1 | 2): CityLayout {
...
export function computeWindowPixels(size = 64, seed = 9): Uint8Array {
...
/** One city: asphalt block pads (streets are the gaps) + building bodies whose side faces
 *  carry a procedural lit-window emissiveMap (night-gated). Every instance follows the
 *  sphere via its own surface normal and a 5-point terrain minimum (v1 technique kept). */
export function buildCityChunk(site: CitySite, planetPos: THREE.Vector3, planetSeed: number, radius: number): CityChunk {
  const layout = computeCityLayout(site.seed, site.tier)
  ...
  interface Placement { x: number; z: number; w: number; d: number; h: number; dir: THREE.Vector3; ground: number }
  /** Shared 5-point terrain anchor with center water gate — used by pads and buildings alike. */
  function place(x: number, z: number, w: number, d: number, h: number): Placement | null {
    ...
    if (!dirC) return null
    return { x, z, w, d, h: h + (centerGround - ground), dir: dirC, ground }
  }

  const padPlacements: Placement[] = []
  for (const p of layout.pads) {
    const placed = place(p.x, p.z, CITY_BLOCK, CITY_BLOCK, 2)
    if (placed) padPlacements.push(placed)
  }
  ...
  const padMat = new THREE.MeshStandardMaterial({ color: 0x14171c, roughness: 0.96, metalness: 0 })
  const pads = new THREE.InstancedMesh(unitBox, padMat, padPlacements.length)

  const windowTexture = new THREE.DataTexture(computeWindowPixels(64, site.seed) as unknown as Uint8Array<ArrayBuffer>, 64, 64, THREE.RGBAFormat)
  ...
  padPlacements.forEach((p, i) => {
    q.setFromUnitVectors(up, p.dir)
    pos.copy(planetPos).addScaledVector(p.dir, p.ground - 2)
