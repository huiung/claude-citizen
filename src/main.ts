...
declare global {
  interface Window {
    __sccShowcase?: {
      ...
      hideChatDraft: () => void
      launch: () => void
      openShipyard: (type: ShipType) => void
      reset: () => void
      setCamera: (pos: [number, number, number], target: [number, number, number], fov?: number) => void
      setCaption: (title: string, body?: string) => void
      setPose: (pos: [number, number, number], target: [number, number, number]) => void
      setQuantumLabel: (text: string | null) => void
      setSafeZone: (active: boolean) => void
      setShip: (type: ShipType) => void
      setupGalaxy: () => { body: { position: [number, number, number]; radius: number; type: string }; ship: [number, number, number] }
      showChatDraft: (text: string) => void
      spawnHostiles: () => void
      startQuantum: (target: [number, number, number]) => void
      state: () => { position: [number, number, number]; quantum: string }
    }
  }
}

if (new URLSearchParams(location.search).has('capture-showcase')) {
  ...
  caption.style.cssText = [
    ...
    'background:rgba(1,6,14,0.70)',
    'box-shadow:0 0 24px rgba(111,232,255,0.10)',
  ...
  style.textContent = `
    #station.capture-showcase {
      justify-content: flex-start;
      background: linear-gradient(90deg, rgba(0,0,0,.90) 0%, rgba(1,10,6,.84) 47%, rgba(0,0,0,.20) 73%, rgba(0,0,0,.05) 100%);
      pointer-events: none;
    }
    #station.capture-showcase .station-card {
      margin-left: 34px;
      align-self: center;
      width: 540px;
    }
    body.capture-showcase #chat-log {
      width: 460px;
      bottom: 128px;
    }
    body.capture-showcase .chat-line {
      font-size: 13px;
      padding: 3px 8px;
    }
  `
  document.head.appendChild(style)
  document.body.classList.add('capture-showcase')
  ...
  window.__sccShowcase = {
    ...
    closeStation() {
      stationMenu.close()
      stationMenu.root.classList.remove('capture-showcase')
      docked = false
    },
    hideChatDraft() {
      chatInputEl.hidden = true
    ...
    launch() {
      ...
      nicknameEl.value = 'CALIBER'
      localStorage.setItem('callsign', 'CALIBER')
      refreshWallet()
      syncHUD()
    },
    openShipyard(type) {
      econ.credits = Math.max(econ.credits, 10000)
      for (const t of SHIP_TYPES) ownedShips.add(t)
      saveHangar()
      dock('refinery')
      stationMenu.root.classList.add('capture-showcase')
      const tab = stationMenu.root.querySelector('[data-tab="shipyard"]') as HTMLButtonElement | null
      tab?.click()
      setPlayerCraft(type)
      syncHUD()
    },
    reset() {
      ...
      for (const f of oreFloats.splice(0)) scene.remove(f.obj)
      chatLines.splice(0).forEach((line) => line.remove())
      ...
      shipMesh.position.copy(ship.position)
      shipMesh.quaternion.copy(ship.quaternion)
      setPlayerCraft('hauler')
      econ.credits = Math.max(econ.credits, 5000)
      playerHealth.hull = playerHealth.max
      nextSpawnAt = Infinity
      refreshWallet()
      syncHUD()
    },
    setCamera(pos, lookAt, fov = 58) {
      captureCameraLocked = true
      camera.position.fromArray(pos)
      target.fromArray(lookAt)
      camera.lookAt(target)
      camera.fov = fov
      camera.updateProjectionMatrix()
    },
    setCaption(title, body = '') {
      caption.innerHTML = `<div style="font-weight:850;font-size:26px;line-height:1.04">${title}</div>${
        body ? `<div style="margin-top:9px;font-weight:700;font-size:14px;line-height:1.35;color:#bfefff">${body}</div>` : ''
      }`
    },
    setPose(pos, lookAt) {
    ...
    setQuantumLabel(text) {
      quantumEl.hidden = text === null
      if (text !== null) quantumEl.textContent = text
    },
    setSafeZone(active) {
      safeEl.hidden = !active
    },
    setShip(type) {
      ownedShips.add(type)
      setPlayerCraft(type)
    ...
    setupGalaxy() {
      const centers = [
        new THREE.Vector3(36000, 5000, -22000),
        new THREE.Vector3(-42000, -6000, -26000),
        new THREE.Vector3(28000, 16000, 34000),
        new THREE.Vector3(-18000, 20000, 42000),
      ]
      let picked: Celestial | null = null
      let center = centers[0]
      for (const c of centers) {
        const candidates = queryCelestials(c, STREAM_RADIUS)
          .filter((b) => b.type === 'planet' || b.type === 'moon' || b.type === 'station')
      ...
      picked ??= {
        id: 'capture.fallback',
        type: 'planet',
        position: new THREE.Vector3(center.x + 18000, center.y - 2000, center.z - 26000),
        radius: 6000,
        seed: 1,
      }
      ...
      ship.velocity.set(0, 0, 0)
      shipMesh.position.copy(ship.position)
      target.copy(picked.position)
      shipMesh.lookAt(target)
      ship.quaternion.copy(shipMesh.quaternion)
      lastStream = -Infinity
      streamCelestials(performance.now() + 1000)
      return {
        body: { position: [picked.position.x, picked.position.y, picked.position.z], radius: picked.radius, type: picked.type },
        ship: [ship.position.x, ship.position.y, ship.position.z],
      }
    },
    showChatDraft(text) {
      chatOpen = true
    ...
    spawnHostiles() {
      removeHostiles()
      const positions = [
        new THREE.Vector3(ship.position.x + 120, ship.position.y + 40, ship.position.z - 210),
        new THREE.Vector3(ship.position.x - 150, ship.position.y - 20, ship.position.z - 260),
        new THREE.Vector3(ship.position.x + 40, ship.position.y + 70, ship.position.z - 320),
      ]
      for (let i = 0; i < positions.length; i++) {
        const pirate = spawnPirate(`showcase-${i}`, positions[i])
        pirates.push(pirate)
        const mesh = buildCraft('interceptor', 0xc0392b)
    ...
    startQuantum(dest) {
      ...
      startTravel(quantum, new THREE.Vector3(...dest))
    },
    state() {
      return {
        position: [ship.position.x, ship.position.y, ship.position.z],
        quantum: quantum.phase,
      }
    },
  }
}
