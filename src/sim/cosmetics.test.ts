...
describe('cosmeticStyle', () => {
  it('maps each recipe to its category', () => {
    ...
    expect(COSMETIC_CATEGORY['comet-wake-kit']).toBe('trail')
...
describe('decodeCosmetics', () => {
  ...
  it('keeps Comet Wake recipe identity when decoding peer cosmetics', () => {
    const styles = decodeCosmetics('comet-wake-kit:epic,,')
    expect(styles).toEqual([
      expect.objectContaining({ recipeId: 'comet-wake-kit', category: 'trail', legendary: false }),
    ])
  })
