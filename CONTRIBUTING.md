...
| `src/render/` | Three.js rendering, procedural generation | Reads sim state, never mutates it. **No asset files in here** — meshes are either built in code or loaded from `public/assets/`, which is generated by `scripts/` except for the CC-BY hero hulls credited in the README. |
